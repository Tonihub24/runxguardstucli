...
// ---------------- Mirror Verification ----------------
func verifyMirror(dir string) {
	fmt.Printf("🔹 Verifying folder: %s\n", dir)

	// Load baseline file
	data, err := os.ReadFile(baselineFile)
	if err != nil {
		fmt.Println("⚠️ Baseline not found. Run init first.")
		return
	}

	var baseline Baseline
	if err := json.Unmarshal(data, &baseline); err != nil {
		fmt.Println("⚠️ Failed to parse baseline:", err)
		return
	}

	// Check each file in baseline
	for _, f := range baseline.Files {
		fullPath := filepath.Join(dir, filepath.Base(f.Path))
		...
		if info.IsDir() {
			...
			hash, _ := calculateFileHash(fullPath)
			if hash == f.Hash {
				fmt.Printf("✅ %s: exists and hash matches\n", filepath.Base(f.Path))
			} else {
				fmt.Printf("⚠️ %s: hash mismatch!\n", filepath.Base(f.Path))
			}
		}
	}
...
case "verify":
	if len(os.Args) < 3 {
		fmt.Println("Usage: runtimeguard verify <directory>")
		return
	}
	verifyMirror(os.Args[2])
